...
(define-record-type (<trie> %make-trie trie?)
  (fields (mutable value %trie-value set-trie-value!)
	  (mutable edge-alist trie-edge-alist set-trie-edge-alist!))
  )
...
(define (trie-value trie)
  (or (%trie-value trie)
      (error 'trie-value "Trie node has no value:" trie)))
...
(define (get-a-value-by-searching trie features)
  (let loop
      ((trie trie)
       ...
       (succeed
        (lambda (value fail)
          ;; (declare (ignore fail))  ignore似乎只是单纯屏蔽参数用的 注释掉 2023年12月22日21:42:31
          value))
       (fail
        (lambda ()
          (error "Unable to match features:" features))))
